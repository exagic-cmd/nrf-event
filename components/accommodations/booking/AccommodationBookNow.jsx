...
const ConfirmationModal = ({ isOpen, onClose, onConfirm, bookingResponse, price }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center p-4">
      <div className="bg-surface rounded-2xl max-w-7xl w-full max-h-[92vh] overflow-y-auto shadow-3xl">
        <div className="p-6 md:p-10">
            ...
            {items.map((item, idx) => {
              ...
              return (
                <div key={idx} className="border border-border rounded-2xl overflow-hidden shadow-sm">
                  <div 
                    className="bg-muted hover:bg-muted transition p-4 md:px-6 flex justify-between items-center cursor-pointer select-none"
                    onClick={() => setExpandedRoomIndex(isExpanded ? -1 : idx)}
                  >
                    <div className="flex items-center gap-3">
                      <Bed className="h-6 w-6 text-primary" />
                      <h4 className="text-lg font-bold text-foreground">Room {idx + 1}: <span className="font-medium text-muted-foreground">{roomType}</span></h4>
                    </div>
                  ...
                  {isExpanded && (
                    <div className="p-4 md:p-6 border-t border-border bg-surface">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        <div className="flex items-center gap-3"><Calendar className="h-5 w-5 text-muted-foreground" /><div><p className="opacity-90 text-sm">Check-in</p><p className="font-bold">{checkIn}</p></div></div>
                        <div className="flex items-center gap-3"><Calendar className="h-5 w-5 text-muted-foreground" /><div><p className="opacity-90 text-sm">Nights</p><p className="font-bold">{nights}</p></div></div>
                        <div className="flex items-center gap-3"><Utensils className="h-5 w-5 text-muted-foreground" /><div><p className="opacity-90 text-sm">Meal</p><p className="font-bold">{mealType}</p></div></div>
                      ...
                      <div className="bg-gradient-to-b from-gray-50 to-white rounded-2xl p-6 mb-8 border border-border">
                        ...
                        <div className="space-y-4">
                          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-7 gap-3">
                            {(Array.isArray(nightCosts) && nightCosts.length > 0
                              ...
                              return (
                                <div key={cIdx} className="bg-surface rounded-xl shadow-sm p-2 flex flex-col items-center justify-center text-center border border-border">
                                  <div className="text-sm text-muted-foreground mb-1">{label}</div>
                                  <div className="text-md font-semibold text-foreground">{currency}{amt.toFixed(2)}</div>
                                </div>
                              );
                            })}
                          </div>
                        ...
                        <div className="flex justify-between items-center pt-5 mt-5 border-t border-border">
                          <span className="text-lg font-semibold text-foreground">Room Total</span>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-surface/0 p-4 rounded-2xl border border-border">
                          ...
                          {generalMessages && generalMessages.length > 0 ? (
                            <div className="space-y-3">
                              {generalMessages.map((m, i) => {
                                ...
                                return (
                                  <div
                                    key={i}
                                    className="p-3 bg-muted rounded-lg border border-border text-sm text-muted-foreground max-h-40 overflow-y-auto"
                            ...
                            <div className="text-sm text-muted-foreground">No general messages provided.</div>
                          )}
                        </div>
                          ...
                          {internalNotes && internalNotes.length > 0 ? (
                            <div className="space-y-3">
                              {internalNotes.map((m, i) => {
                                ...
                                return (
                                  <div key={i} className="p-3 bg-muted rounded-lg border border-border text-sm text-muted-foreground max-h-40 overflow-y-auto">
                            ...
                            <div className="text-sm text-muted-foreground">No internal notes available.</div>
                          )}
                        </div>
                      </div>
          ...
          <div className="flex justify-between items-center pt-5 border-t-4 border-double border-border">
            <span className="text-1xl font-bold text-foreground">Grand Total Amount</span>
      ...
      <div className="bg-surface rounded-2xl max-w-md w-full shadow-xl p-8 text-center">
        ...
        <div className="flex gap-4">
          <button
            ...
            className="flex-1 bg-secondary hover:bg-secondary text-foreground font-semibold py-3 rounded-lg transition"
          >
            No, Keep Existing
          </button>
          <button
  ...
  return (
    <>
      <div className="bg-surface rounded-lg p-6">
        <h2 className="font-bold text-md lg:text-md text-surface-foreground mb-6">
          Guest Information
        </h2>
          ...
          {rooms.map((room, roomIdx) => {
            ...
            return (
              <div key={roomIdx} className="rounded-lg p-0 md:p-4">
                <h3 className="font-bold text-md lg:text-md text-surface-foreground mb-5">
                ...
                {roomGuests.adults.map((adult, i) => (
                  <div
                    key={`adult-${i}`}
                    className="bg-muted  p-2 grid grid-cols-1 md:grid-cols-3 gap-4 mt-2"
                  >
                    {/* Title */}
                    <div>
                ...
                {roomGuests.children.map((child, i) => (
                  <div
                    key={`child-${i}`}
                    className="bg-secondary rounded-lg p-4 grid grid-cols-1 md:grid-cols-4 gap-4 mb-4 mt-2"
                  >
                    {/* Title */}
                    <div>
                      <label className="block text-surface-foreground text-sm font-medium mb-2">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={child.age}
                        readOnly
                        className="w-full px-3 py-2 bg-secondary border border-border rounded-md text-surface-foreground placeholder-gray-400 focus:outline-none cursor-not-allowed"
                      />
                    </div>
                  </div>
                ))}
