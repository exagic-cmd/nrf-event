...
const TourVariants = forwardRef(({ groupProducts, onVariantSelect }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className="mx-4 sm:mx-6 lg:mx-12 mb-12 sm:mb-24 bg-surface shadow-xl rounded-2xl overflow-hidden scroll-mt-24"
    >
      {/* Mobile Cards View */}
      <div className="block lg:hidden">
        <div className="p-4 bg-muted border-b">
          <h3 className="text-lg font-bold text-foreground">{t("choose_tour")}</h3>
        </div>
        <div className="divide-y divide-gray-200">
          {groupProducts.map((variant) => (
            <div key={variant.group_product_id} className="p-4 space-y-3">
              <div>
                <h4 className="font-bold text-foreground text-base">{variant.Title}</h4>
                <p className="text-sm text-muted-foreground mt-1">{variant.Description}</p>
              </div>
              {variant.highlights && variant.highlights.length > 0 && (
                <div className="space-y-2">
                  <h5 className="text-sm font-medium text-muted-foreground">{t("highlights")}</h5>
              ...
              <div className="flex items-center justify-between pt-2">
                <div>
                  <span className="text-lg font-bold text-foreground">{variant.Starting_price}</span>
                  <span className="text-sm text-muted-foreground ml-1">{t("per_person")}</span>
                </div>
                <button
                  onClick={() => onVariantSelect(variant)}
                  className="bg-primary hover:bg-primary text-white text-sm px-4 py-2 rounded-xl font-semibold shadow"
                >
                  {t("choose")}
                </button>
              </div>
      ...
      <div className="hidden lg:block overflow-x-auto">
        <table className="w-full text-sm text-left text-muted-foreground">
          <thead className="bg-muted text-foreground text-sm uppercase font-semibold">
            <tr>
          ...
          <tbody>
            {groupProducts.map((variant) => (
              <tr key={variant.group_product_id} className="border-b hover:bg-orange-50 transition">
                <td className="p-4">
                  <div className="font-bold text-foreground">{variant.Title}</div>
                ...
                <td className="p-4 font-bold text-foreground text-lg">
                  {variant.Starting_price}
                  <span className="text-sm font-normal text-muted-foreground"> {t("per_person")}</span>
                </td>
                <td className="p-4 text-center">
                  <button
                    onClick={() => onVariantSelect(variant)}
                    className="bg-primary text-white text-sm px-5 py-2 rounded-xl font-semibold shadow"
                  >
                    {t("choose")}
                  </button>
                </td>
