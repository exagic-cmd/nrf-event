...
const TourAccordion = ({ apiData }) => {
  ...
  return (
    <div className="bg-surface rounded-lg p-5  shadow-sm">
      {/* Included Section */}
      <h3 className="font-semibold text-primary mb-3 text-lg">{t("whatsIncluded")}</h3>
      <ul className="space-y-2 mb-6">
        {inclusions.length > 0 ? (
          inclusions.map((item, i) => (
            <li key={i} className="flex items-start gap-2 text-surface-foreground text-sm">
              <CheckCircle className="text-primary mt-1 flex-shrink-0" size={16} />
              <span>{item}</span>
            </li>
          ))
        ) : (
          <p className="text-muted-foreground text-sm">{t("noInclusionsAvailable")}</p>
        )}
      </ul>

      {/* Excluded Section */}
      <h3 className="font-semibold text-primary mb-3 text-lg">{t("whatsExcluded")}</h3>
      <ul className="space-y-2">
        {exclusions.length > 0 ? (
          exclusions.map((item, i) => (
            <li key={i} className="flex items-start gap-2 text-surface-foreground text-sm">
              <XCircle className="text-primary mt-1 flex-shrink-0" size={16} />
              <span>{item}</span>
            </li>
          ))
        ) : (
