...
export default function LocationTransition({ prevLocation, nextLocation, travelInfo, onNext, title, translation, details }) {
  return (
    <div className="bg-surface max-w-xl md:max-w-7xl  text-surface-foreground p-2 rounded-lg shadow-lg  mx-auto my-4 ">
    
     <h2 className="text-lg lg:text-xl text-start mb-2 text-surface-foreground font-semibold pl-2">
  {title}
</h2>

{/* Travel info row */}
{details && (
  <div className="flex items-center gap-6 text-foreground text-sm pl-2">
    ...
    {details.travel_mode && (
      ...
      <div className="space-y-4 md:space-y-6">
        {travelInfo && travelInfo.length > 0 ? (
          <div>
            {/* <h3 className="text-md font-semibold text-muted-foreground">Travel Note</h3> */}
            {travelInfo.map((info, index) => (
              <div key={index} className=" mt-6">
                ...
                {info.guideFallback && (
                  <p className="text-sm text-foreground mb-2">No audio available for the selected guide for this segment.</p>
                )}
          ...
          <div>
            <p className="text-sm text-foreground mb-2">No audio available for this part of the tour.</p>
          </div>
        )}
   ...
   {/* <div className="lg:mx-2 mx-1 flex justify-end">
       <button
        onClick={onNext}
        className=" bg-brand-secondary  hover:bg-[#e1b97b] text-surface-foreground font-bold py-3 px-8 rounded-lg mt-8 transition duration-300 ease-in-out"
      >
         Next Stop 
      </button>
   </div> */}
